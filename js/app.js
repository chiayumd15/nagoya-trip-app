...
function renderHome() {
  ...
  if (dl > 0) head = `<div class="card tape-r tilt-l"><span class="doodle">✈️</span><div class="countdown"><div class="n">${dl}</div><div><div>天後出發${m ? `（${T.GROUPS[m.group].name}）` : ''}</div><div class="small muted">${m && m.group === 'chen' ? '10/13（二）15:55 桃園 → 名古屋' : '10/10（六）10:15 桃園 → 大阪'}</div></div></div>${m ? '' : `<div class="small muted" style="margin-top:6px">點右上角「我是誰？」選自己，行程、早餐、房號、分帳會變成你的視角。</div>`}</div>`;
...
function renderMoney() {
  ...
  <div class="card"><div class="row between"><div><div class="small muted">總花費</div><div class="money-big">${fmtJPY(total)}</div><div class="tiny muted">≈ ${fmtTWD(total * rate())} · 楊家 ${fmtJPY(byGroup.yang)} / 陳家 ${fmtJPY(byGroup.chen)}</div></div>${m ? `<div style="text-align:right"><div class="small muted">${m.name} 的份</div><div class="money-big">${fmtJPY(mine)}</div><div class="tiny muted">≈ ${fmtTWD(mine * rate())}</div></div>` : ''}</div>
...
function addExpModal(pre = {}) {
  ...
  <label class="lbl">分給誰（點選切換）</label><div class="btnrow" style="margin:0 0 6px"><button type="button" class="btn sm" data-sel="all">全部 11 人</button><button type="button" class="btn sm" data-sel="yang">只有楊家</button><button type="button" class="btn sm" data-sel="chen">只有陳家</button><button type="button" class="btn sm ghost" data-sel="none">清空</button></div><div class="people-grid" id="exPeople">${people}</div>
...
旅行：2026/10/10–10/19，共 11 人兩家族。楊家 5 人（楊才廣、黃思婷、楊家喻、張志郁、楊書竣）10/10 JX822 TPE10:15→KIX14:00，先玩大阪京都，10/13 新幹線到名古屋。姑姑家 6 人（陳永康、楊淑媛、陳玟玲、高魁澤、陳玟潔、陳玟綺）10/13 星宇 TPE15:55→NGO18:45。全員 10/19 JX839 NGO19:55→TPE22:00。
...
function renderMore() {
  ...
  const people = T.PEOPLE.map(p => `<tr><td><span class="badge-g ${p.group}">${T.GROUPS[p.group].short}</span>${p.name}</td><td>房 ${p.room}</td><td>${p.breakfast ? '🍳 含' : '— 不含'}</td><td class="tiny">${p.group === 'yang' ? '10/10 JX822' : '10/13 星宇'}</td></tr>`).join('');
  ...
  <div class="card"><h2>👨‍👩‍👧‍👦 11 人名單・房號・航班</h2><div class="tablewrap"><table><thead><tr><th>姓名</th><th>房</th><th>早餐</th><th>去程</th></tr></thead><tbody>${people}</tbody></table></div><div class="tiny muted">分組是依姓氏推的：楊家 5 人、姑姑家 6 人。若有誤請告訴家喻改 data.js。</div><div class="btnrow"><button class="btn sm" data-act="me">切換我是誰</button></div></div>
...
function openMe() {
  openModal(`<h2>你是誰？</h2><div class="small muted">選了之後，行程、早餐、房號、記帳、聊天都會用你的身分。</div><h3 style="margin-top:10px">楊家</h3><div class="people-grid">${T.PEOPLE.filter(p => p.group === 'yang').map(p => `<button class="pbtn yang ${state.me === p.id ? 'on' : ''}" data-act="setme" data-id="${p.id}">${p.name}</button>`).join('')}</div><h3 style="margin-top:10px">姑姑家</h3><div class="people-grid">${T.PEOPLE.filter(p => p.group === 'chen').map(p => `<button class="pbtn chen ${state.me === p.id ? 'on' : ''}" data-act="setme" data-id="${p.id}">${p.name}</button>`).join('')}</div>`);
}
