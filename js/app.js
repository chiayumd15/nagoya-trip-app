...
function renderHome() {
  ...
  if (dl > 0) head = `<div class="card tape-r tilt-l"><span class="doodle">✈️</span><div class="countdown"><div class="n">${dl}</div><div><div>天後出發${m ? `（${T.GROUPS[m.group].name}）` : ''}</div><div class="small muted">${m && m.group === 'chen' ? '10/13（二）JX838 15:55 桃園 → 名古屋' : '10/10（六）10:15 桃園 → 大阪'}</div></div></div>${m ? '' : `<div class="small muted" style="margin-top:6px">點右上角「我是誰？」選自己，行程、早餐、房號、分帳會變成你的視角。</div>`}<div class="btnrow"><a class="btn sm" target="_blank" rel="noopener" href="https://www.vjw.digital.go.jp/">🛂 Visit Japan Web</a><a class="btn sm ghost" target="_blank" rel="noopener" href="https://www.starlux-airlines.com/zh-TW/check-in-fly/check-in-now/online-check-in">✈️ 星宇線上報到</a><button class="btn sm ghost" data-tab="more">🧳 行李清單</button></div></div>`;
...
function renderMoney() {
  ...
  <div class="card"><div class="row between"><div><div class="small muted">總花費</div><div class="money-big">${fmtJPY(total)}</div><div class="tiny muted">≈ ${fmtTWD(total * rate())}</div><div class="small" style="margin-top:4px">楊 <b>${fmtJPY(byGroup.yang)}</b> <span class="tiny muted">≈ ${fmtTWD(byGroup.yang * rate())}</span> ／ 陳 <b>${fmtJPY(byGroup.chen)}</b> <span class="tiny muted">≈ ${fmtTWD(byGroup.chen * rate())}</span></div></div>${m ? `<div style="text-align:right"><div class="small muted">${m.name} 的份</div><div class="money-big">${fmtJPY(mine)}</div><div class="tiny muted">≈ ${fmtTWD(mine * rate())}</div></div>` : ''}</div>
...
function addExpModal(pre = {}) {
  ...
  <label class="lbl">分給誰（點選切換）</label><div class="btnrow" style="margin:0 0 6px"><button type="button" class="btn sm" data-sel="all">全部 11 人</button><button type="button" class="btn sm" data-sel="yang">楊家 5 人</button><button type="button" class="btn sm" data-sel="chen">陳家 6 人</button><button type="button" class="btn sm ghost" data-sel="none">清空</button></div><div class="people-grid" id="exPeople">${people}</div>
...
function renderMore() {
  ...
  const ckT = i => typeof i === 'string' ? i : i.t, ckU = i => typeof i === 'string' ? '' : (i.url || '');
  const cats = [...T.CHECKLIST.map(c => ({ cat: c.cat, items: c.items.filter(i => !hidden[c.cat + '|' + ckT(i)]) })), ...(state.customChecks.length ? [{ cat: '✍️ 自己加的', items: state.customChecks }] : [])].filter(c => c.items.length);
  const allItems = []; cats.forEach(c => c.items.forEach(i => allItems.push(c.cat + '|' + ckT(i))));
